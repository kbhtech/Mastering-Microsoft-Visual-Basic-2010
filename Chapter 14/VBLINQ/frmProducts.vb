Public Class frmProducts
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        frmProduct.bttnCreate.Text = "Add Product"
        If frmProduct.ShowDialog = Windows.Forms.DialogResult.OK Then
            ctx = New NWindDataContext
            Dim P As New Product
            Dim productName As String = frmProduct.txtProduct.Text
            ...
            Try
                productPrice = Decimal.Parse(frmProduct.txtPrice.Text)
            Catch ex As Exception
                productPrice = Nothing
            End Try

            Dim productCategoryID? As Integer
            If frmProduct.cbCategories.SelectedIndex > -1 Then
                productCategoryID = CType(frmProduct.cbCategories.SelectedItem, Category).CategoryID
            Else
                productCategoryID = Nothing
            End If

            Dim productSupplierID? As Integer
            If frmProduct.cbSuppliers.SelectedIndex > -1 Then
                productSupplierID = CType(frmProduct.cbSuppliers.SelectedItem, Supplier).SupplierID
            Else
                productSupplierID = Nothing
            End If

            Dim Packaging As String
            If frmProduct.txtQtyUnits.Text.Trim.Length > 0 Then
                Packaging = frmProduct.txtQtyUnits.Text.Trim
            Else
                Packaging = Nothing
            End If
    ...
    Private Sub lvProducts_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lvProducts.DoubleClick
        ...
        frmProduct.txtProduct.Text = product.ProductName
        frmProduct.txtPrice.Text = (Convert.ToDecimal(product.UnitPrice)).ToString("#,###.00")
        frmProduct.txtQtyUnits.Text = product.QuantityPerUnit
        frmProduct.cbSuppliers.Text = ""
        If product.Supplier IsNot Nothing Then frmProduct.cbSuppliers.SelectedItem = product.Supplier
        frmProduct.cbCategories.Text = ""
        If product.Category IsNot Nothing Then frmProduct.cbCategories.SelectedItem = product.Category
        frmProduct.bttnCreate.Text = "Submit changes"
        If frmProduct.ShowDialog = Windows.Forms.DialogResult.OK Then
            product.Category = frmProduct.cbCategories.SelectedItem
            product.Supplier = frmProduct.cbSuppliers.SelectedItem
            product.ProductName = frmProduct.txtProduct.Text
            product.QuantityPerUnit = frmProduct.txtQtyUnits.Text
            product.UnitPrice = frmProduct.txtPrice.Text
            ctx.SubmitChanges()
        End If
